...
referenceLine::referenceLine(){
    ...
    // subscriber
    routing_sub_ = n_.subscribe("/tprouteplan", 10, &referenceLine::routingCallback, this);
    ...
    rviz_egocar_pub_ = n_.advertise<visualization_msgs::MarkerArray>("/rviz_egocar", 10);      // 在rviz中显示自车
...
void referenceLine::routingCallback(const map_msgs::RoutePlan &routing){
    routing_waypoints_ = Eigen::MatrixXd::Zero(1, 3);
    // 确保一开始只订阅一次
    setlocale(LC_ALL,"");
    if (routing_waypoint_flag_.size() < 1)
    {
        routing_waypoint_flag_.push_back(routing.point.xg);
        // for (int i = 0; i < routing.poses.size(); ++i)
        // {
          routing_waypoints_(0, 0) = routing.point.xg;
          routing_waypoints_(0, 1) = routing.point.yg;
          routing_waypoints_(0, 2) = routing.point.heading;
        // }
        // 参考线的平滑计算只计算一次
...
void clear_obstacles_squares(visualization_msgs::Marker &marker)
{
...
void referenceLine::turn_ego_into_squares(visualization_msgs::Marker &marker)
{
  marker.pose.orientation = tf::createQuaternionMsgFromYaw(gps_.Yaw);
  marker.header.frame_id = "world";
  marker.header.stamp = ros::Time::now();
  marker.ns = "basic_shapes";
  marker.id = 100;
  marker.type = visualization_msgs::Marker::CUBE;
  marker.action = visualization_msgs::Marker::ADD;
  marker.pose.position.x = gps_.Position_x;
  marker.pose.position.y = gps_.Position_y;
  marker.pose.position.z = 0;
  marker.scale.x = 3.3;
  marker.scale.y = 1.6;
  marker.scale.z = 1.8;
...
// rviz可视化,显示参考线平滑后的轨迹
void referenceLine::RvizRunReferencePublish(const nav_msgs::Path &path_point)
{

    geometry_msgs::PoseArray pose_stamp;
    pose_stamp.poses.resize(path_point.poses.size());
    pose_stamp.header.frame_id = "world";
    pose_stamp.header.stamp = ros::Time::now();
    for (int i = 0; i < path_point.poses.size(); ++i)
    {
      pose_stamp.poses[i].position.x = path_point.poses[i].pose.position.x;
      pose_stamp.poses[i].position.y = path_point.poses[i].pose.position.y;
      pose_stamp.poses[i].position.z = 0;  
    }
    rviz_reference_pub_.publish(pose_stamp);  
}


void  referenceLine::run() {
  
  Obstacle ob(true); // 实例化类，true 表示通过构造函数订阅话题 /tpperception
  // 当订阅到消息后，通过 setObstacles() 
  ros::Rate loop_rate(10);
  while (ros::ok())
  {
      
    // 参考线平滑完毕，计算参考点的kappa、theta
    if (referenceline_.poses.size() > 0)
    {
      referencePointsCalc(referenceline_);

      // 参考线参考线平滑之后的形状,持续发布话题 /rviz_global_path
      RvizRunReferencePublish(referenceline_);
    }

    // 障碍物存储在 obstacles 中                           已订阅
    std::vector<const Obstacle *> obstacles;   // 在plan() 函数中再对obstacles 进行赋值
    // for (size_t i = 0; i < AllObstacle.size(); ++i)
    // {
    //   obstacles.emplace_back(&AllObstacle[i]);
    // }

    // ROS_INFO("AllObstacle.size() :%ld", AllObstacle.size()); // gt碰撞检测调试：打印 有几个障碍物

    // 确保收到数据 gps 和 reference_points 之后再进行规划
    if (reference_points.size() > 0 && gps_flag_.size() > 0)
    {
      ROS_INFO("正在运行planning！");
      ...
      // 2.轨迹规划
      PlanningTarget planning_target(Config_.default_cruise_speed, accumulated_s); // EM巡航速度 11m/s，参考线纵向累积距离 referencePointsCalc() 函数中计算出来的
      lon_decision_horizon = accumulated_s[accumulated_s.size() - 1];   // 前视距离，未使用该值
      ...
      // ROS_INFO("best_stitch_pre_trajectory_.size = %ld", pre_trajectory_.size());
      ...
      for (int i = 0; i < pre_trajectory_.size();++i){
        ...
        // TrajectoryPo int_d.z = pre_trajectory_[i].z;
      ...
      // 4.将 planning 规划的局部轨迹，通过话题 /rviz_local_path 发送到 rviz
      ...
      rviz_pub_.publish(traj_points_);

      // // 将局部轨迹打印到 txt 文件中
      // std::ofstream myout("/home/gt/桌面/catkin_ws/log.txt");
      // for(int i = 0; i < traj_points_.poses.size(); ++i) {
      //     myout << traj_points_.poses[i].pose.position.x << " " << traj_points_.poses[i].pose.position.y << std::endl;
      // }
      // myout.close();
      // ROS_INFO("over\n");
    ...
    // 在rviz上显示障碍物，持续发布话题 /rviz_obstacles       已发布
    if (obstacles.size() > 0)
    {
    ...
    //   visualization_msgs::Marker marker;
    //   clear_obstacles_squares(marker);
    //   obstacle_MarkerArray.markers.push_back(marker);
   ...
   if (1)  {
      ...
      visualization_msgs::Marker marker;
      turn_ego_into_squares(marker); // id 100
      ego_MarkerArray.markers.push_back(marker);

      rviz_egocar_pub_.publish(ego_MarkerArray);
   }
...
void referenceLine::referenceLine_split(Eigen::MatrixXd &hdmap_way_points) {
  // 参数：（输入）routing 数据，(输出) 插值后的轨迹点， 插值点两点的距离， 两点距离太长的话就进行插点
  average_interpolation(hdmap_way_points, path_point_after_interpolation_, 0.5, 1); // 原先的参数 0.2 0.6
...
void referenceLine::referencePointsCalc(const nav_msgs::Path &path_point)
{
    ...
    {
      // reference_path.first.clear();    // 没有用到这个变量 reference_path
...
std::vector<TrajectoryPoint> referenceLine::plan_start_point(double &current_time) {
    if (this->is_first_run){
        ...
        double x_init = gps_.Position_x;// 定位
        double y_init = gps_.Position_y;
        double z_init = 0;
        double v_init = 0;
        double a_init = 0;
        double theta_init = gps_.Yaw;
        double kappa_init = 0;
...
//        std::cout << "非第一次运行且有历史轨迹" << std::endl;
        double x_cur = gps_.Position_x;// 定位
        double y_cur = gps_.Position_y;
        double theta_cur = gps_.Yaw;
        double kappa_cur = 0;
        double vx_cur = gps_.Velocity_x;
        double vy_cur = gps_.Velocity_y;
        double ax_cur = gps_.Accel_x;
        double ay_cur = gps_.Accel_y;
        double dt = 0.1;
        int index = 0;
...
//        printf("lon_err = %f m\n", lon_err);
//        printf("lat_err = %f m\n", lat_err);
        // 如果 y 误差大于2.5或者 x 误差大于0.5认为控制没跟上      // 应该用真正的s l 横纵向上的误差
        if (lon_err > 50 || lat_err > 50 ){    // if (lon_err > 2.5 || lat_err > 0.5)    6 
            ROS_ERROR("out of control, use dynamic to deduce");
...
bool referenceLine::is_update_dynamic(nav_msgs::Path &trj_point_array, int size)
{
  ...
  is_update = false;
  //车的当前位置
  double pp_x = gps_.Position_x;
  double pp_y = gps_.Position_y;
