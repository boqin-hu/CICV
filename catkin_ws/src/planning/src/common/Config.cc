...
#include <string>
#include <vector>
#include <cstdlib> // 包含头文件<cstdlib>来使用getenv


Param_Configs::Param_Configs()
{
  // 绝对文件路径
  // YAML::Node config = YAML::LoadFile("/home/gt/桌面/catkin_ws/src/planning/src/common/configs.yaml");

  // 相对文件路径
  std::string currentWorkingDirectory = getenv("PWD"); // 获取当前工作目录的路径    /home/gt/桌面/catkin_ws
  std::string relativeFilePath = "/src/planning/src/common/configs.yaml"; // 使用相对路径附加到当前工作目录
  std::string fullPath = currentWorkingDirectory + "/" + relativeFilePath;
 

  // std::cout << " currentWorkingDirectory " << currentWorkingDirectory  << std::endl;
  // std::cout << " relativeFilePath " << relativeFilePath  << std::endl;
  // std::cout << " fullPath " << fullPath  << std::endl;

 YAML::Node config = YAML::LoadFile(fullPath); // 加载配置文件
