                             ...
                             const double t_start, const double t_end, const std::array<double, 3> &init_d)
{
  ...
  path_range_.first = s_start; // 初始纵向位置
  path_range_.second = s_end; // 前瞻位置（初始位置前200米）
  time_range_.first = t_start; // 初始时间
  time_range_.second = t_end; // 最大预测时间8s
  init_d_ = init_d; // 初始横向位置

  SetupObstacles(obstacles, discretized_ref_points); // 参数：障碍物列表， 平滑后的参考点列表
}
...
// 对参考线上的每个障碍物在每个时间点设定位置标定框
void PathTimeGraph::SetupObstacles(
    const std::vector<const Obstacle *> &obstacles,
    const std::vector<ReferencePoint> &discretized_ref_points)
{
  // 1.有50个时间点
  const double total_time = Config_.FLAGS_prediction_total_time; // FLAGS_prediction_total_time 是 PRediction 模块中障碍物轨迹预测总时间，5s
  uint32_t num_of_time_stamps_ = static_cast<uint32_t>(std::floor(total_time / Config_.eval_time_interval)); // eval_time_interval 其实就是Prediction模块中障碍物两个预测轨迹点的时间差，0.1s

  // 2.遍历障碍物列表
  for (const Obstacle *obstacle : obstacles)
  {
    if (obstacle->IsVirtual()) // 如果障碍物是虚拟的（Virtual），则跳过
    {
      continue;
    }
    // 3.静态障碍物
...
//            << "\n";
  ...
  // 查看障碍物的ST 图有没有存上  关键点： path_time_obstacle_map_
...
void PathTimeGraph::SetStaticObstacle(const Obstacle *obstacle,
                                      const std::vector<ReferencePoint> &discretized_ref_points)
{
  int obstacle_id = obstacle->obstacle_id; //赋值id
...
//  std::cout << "static obs polygon size = " << obstacle->polygon_points.size() << std::endl;
  ...
  // SL图排除这些障碍物 ：静态障碍物在我range的前方、后方，同时静态障碍物在本车车道外；
  if (sl_boundary.start_s_ > path_range_.second || // path_range_.second 为前瞻距离
...
//              << "\n";
  ...
  std::cout << "Obstacle [" << obstacle_id << "] is within SL." << std::endl; // gt:SL图调试
                                       ...
                                       const std::vector<ReferencePoint> &discretized_ref_points)
{
  ...
  while (relative_time < time_range_.second) // 8s
  {
    ...
    //道路宽度定死先
    double left_width = Config_.FLAGS_default_reference_line_width * 0.5;   // 改动 0.4
    double right_width = Config_.FLAGS_default_reference_line_width * 0.5;
    ...
    std::cout << "Obstacle [" << obstacle->obstacle_id << "] is within ST." << std::endl; // ST图调试
...
//障碍物是否已经构建图
bool PathTimeGraph::IsObstacleInGraph(const int &obstacle_id)
{
...
STPoint PathTimeGraph::SetPathTimePoint(const int &obstacle_id,
