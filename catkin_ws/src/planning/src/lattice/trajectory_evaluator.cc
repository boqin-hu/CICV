  ...
  if (!lon_trajectories.empty() && !lat_trajectories.empty())
  {
    ...
    for (const auto &lon_trajectory : lon_trajectories)
    {
      ...
      for (const auto &lat_trajectory : lat_trajectories)
      {
        ...
        cost_queue_.emplace(Trajectory1dPair(lon_trajectory, lat_trajectory), cost);//  从小到大排序
      }
    }
//    std::cout << "size:" << lat_trajectories.size() << std::endl;

    // ROS_WARN("cost_queue_.size() = %d\n", cost_queue_.size());
                                     ...
                                     const PtrTrajectory1d &lat_trajectory) const
{
  ...
  double cost_total_result = 
        lon_objective_cost * Config_.FLAGS_weight_lon_objective +  // 纵向速度     10 
        lon_jerk_cost * Config_.FLAGS_weight_lon_jerk + // 纵向加速度
        lon_collision_cost * Config_.FLAGS_weight_lon_collision + // 纵向碰撞     5
        centripetal_acc_cost * Config_.FLAGS_weight_centripetal_acceleration + // 向心加速度   1.5
        lat_offset_cost * Config_.FLAGS_weight_lat_offset + // 横向偏移      6.0
        lat_comfort_cost * Config_.FLAGS_weight_lat_comfort ; // 横向舒适性    0.5

    // double cost_total_result = 
    //     lon_objective_cost * Config_.FLAGS_weight_lon_objective +  // 纵向速度     10 -> 3
    //     lon_collision_cost * Config_.FLAGS_weight_lon_collision + // 纵向碰撞     3 -> 10
    //     centripetal_acc_cost * Config_.FLAGS_weight_centripetal_acceleration + // 向心加速度   3.5
    //     lat_offset_cost * Config_.FLAGS_weight_lat_offset;  // 横向偏移      2.0


  // // 打印各个代价值！！！  百分比显示
  // if (lon_collision_cost != 0) {
  //     std::cout << "  " << (lon_objective_cost * Config_.FLAGS_weight_lon_objective / cost_total_result) *100 ;
  //     std::cout << "%   " << (lon_jerk_cost * Config_.FLAGS_weight_lon_jerk / cost_total_result) *100 ;
  //     std::cout << "%   " << (lon_collision_cost * Config_.FLAGS_weight_lon_collision / cost_total_result) *100 ;
  //     std::cout << "%   " << (centripetal_acc_cost * Config_.FLAGS_weight_centripetal_acceleration / cost_total_result) *100 ;
  //     std::cout << "%   " << (lat_offset_cost * Config_.FLAGS_weight_lat_offset / cost_total_result) *100 ;
  //     std::cout << "%   " << (lat_comfort_cost * Config_.FLAGS_weight_lat_comfort / cost_total_result) *100 ;
  //     std::cout << "%   " << std::endl;
  // }


  return cost_total_result;
}
