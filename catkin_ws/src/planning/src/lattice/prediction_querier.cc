...
// 初始化PredictionQuerier对象，将传入的障碍物信息保存在类的成员变量中，以便后续的障碍物查询操作使用
PredictionQuerier::PredictionQuerier(
    const std::vector<const Obstacle *> &obstacles,
    const std::shared_ptr<std::vector<PathPoint>> &ptr_reference_line) // PathPoint 类型的 参考线的点
...
double PredictionQuerier::ProjectVelocityAlongReferenceLine(
    const int &obstacle_id, const double s, const double t) const
{
