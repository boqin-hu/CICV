...
std::vector<std::array<double, 3>>  PathMatcher::GetPathFrenetCoordinate(const std::vector<ReferencePoint> &reference_points, 
                                                           const std::vector<const Obstacle *> &obstacles) {

  std::vector<std::array<double, 3>> relative_coordinate;

  for (int i = 0; i < obstacles.size(); ++i) {
    auto matched_path_point = MatchToPath(reference_points, obstacles[i]->centerpoint.position.x,  
                                                            obstacles[i]->centerpoint.position.y);

    double rtheta = matched_path_point.heading_;
    double rx = matched_path_point.x_;
    double ry = matched_path_point.y_;

    double delta_x = obstacles[i]->centerpoint.position.x - rx;
    double delta_y = obstacles[i]->centerpoint.position.y - ry;
    double side = std::cos(rtheta) * delta_y - std::sin(rtheta) * delta_x;

    std::array<double, 3> relative_coord;
    relative_coord[0] = matched_path_point.accumulated_s_;
    relative_coord[1] = std::copysign(std::hypot(delta_x, delta_y), side);
    relative_coord[2] = rtheta;

    relative_coordinate.emplace_back(relative_coord);
