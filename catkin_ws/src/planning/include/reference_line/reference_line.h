...
private:
    ...
    void RvizRunReferencePublish(const nav_msgs::Path &path_point);

	void turn_ego_into_squares(visualization_msgs::Marker &marker);

private:
	// handle
	ros::NodeHandle n_;
	// publisher
  	ros::Publisher trajectory_pub_, rviz_pub_, rviz_obstacle_pub_,rviz_reference_pub_, rviz_egocar_pub_;
  	// subscriber
	...
	nav_msgs::Path traj_points_;	// 局部规划轨迹,nav_msgs类型
	...
	int which_planners;
	bool RvizRunReferencePublish_flag = true; // 发布rviz全局路径
	/*
