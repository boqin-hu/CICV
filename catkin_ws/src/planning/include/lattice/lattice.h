...
public:
    ...
    DiscretizedTrajectory plan(
      ...
      std::vector<const Obstacle *> &obstacles, // const std::vector<const Obstacle *> &obstacles,
  ...
  void RvizTrajectoryCluster(const DiscretizedTrajectory &path_point);
  void RvizPreBoundingRectangles(const std::vector<Box2d> &pre_bounding_rectangles, double obstacles_size);

private:
    // handle
    ros::NodeHandle n_;
    // publisher
  	ros::Publisher trajectory_pub_, rviz_cluster_pub_, rviz_sample_pub_, rviz_pre_bounding_rectangles_pub_;
  	// subscriber
