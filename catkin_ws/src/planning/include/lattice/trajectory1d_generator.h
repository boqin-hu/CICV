...
public:
  ...
  void GenerateTrajectoryBundles(const PlanningTarget &planning_target,
                                 ...
                                 const std::vector<std::array<double, 3>> &obs_frenet_state, const ReferencePoint &matched_point);

  void GenerateLateralTrajectoryBundle(std::vector<std::shared_ptr<Curve1d>> *ptr_lat_trajectory_bundle, 
                                        const std::vector<const Obstacle *> &obstacles,
