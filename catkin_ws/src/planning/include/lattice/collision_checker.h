...
public:
  ...
  std::vector<Box2d> PreBoundingRectangles() const {
    return predicted_bounding_rectangles_;
  }
...
private:
  ...
  bool IsObstacleOutOfEgoVehicle(const Obstacle *obstacle, const double ego_vehicle_s,
                                                    const std::vector<ReferencePoint> &discretized_reference_line);

private:
  std::vector<Box2d> predicted_bounding_rectangles_;
