...
class RadarObjectList {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('radarobjects')) {
        this.radarobjects = initObj.radarobjects
      }
      else {
        this.radarobjects = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [radarobjects]
    // Serialize the length for message field [radarobjects]
    bufferOffset = _serializer.uint32(obj.radarobjects.length, buffer, bufferOffset);
    obj.radarobjects.forEach((val) => {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize message field [radarobjects]
    // Deserialize array length for message field [radarobjects]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.radarobjects = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.radarobjects[i] = RadarObject.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.radarobjects.forEach((val) => {
  ...
  static Resolve(msg) {
    ...
    if (msg.radarobjects !== undefined) {
      resolved.radarobjects = new Array(msg.radarobjects.length);
      for (let i = 0; i < resolved.radarobjects.length; ++i) {
        resolved.radarobjects[i] = RadarObject.Resolve(msg.radarobjects[i]);
      }
    }
    else {
      resolved.radarobjects = []
    }
